...
const uint8_t* BitmapFont::glyph(char c) const {
    int idx = static_cast<int>(c) - firstChar;
    if (idx < 0 || idx >= charCount) return nullptr;
    return data + idx * glyphHeight;
}

uint16_t BitmapFont::measureWidth(const char* text) const {
    uint16_t w = 0;
    while (*text) {
        if (glyph(*text)) w += glyphWidth;
        text++;
    }
    return w;
}

// 5x7 monospace font (space through '~', 95 glyphs)
// Each glyph is 5 pixels wide x 7 pixels tall, stored row-major (1 byte per row)
static const uint8_t FONT_5X7_DATA[] = {
    ...
    0x00, 0x48, 0x7A, 0x40, 0x00, 0x00, 0x00,  // i
...
static const BitmapFont DEFAULT_FONT = {
    FONT_5X7_DATA,
    5,   // glyphWidth
    7,   // glyphHeight
    0x20, // firstChar (space)
    95,   // charCount (space through ~)
};

const BitmapFont& BitmapFont::defaultFont() {
    return DEFAULT_FONT;
}
